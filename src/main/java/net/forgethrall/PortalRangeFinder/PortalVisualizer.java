...
//		voxels = new boolean[256+portalSize.getX()][256][256+portalSize.getY()];
	...
	private void generateRangeMeshOverworld() {
		...
		int x0 = portalOrigin.getX()*8-128;
		int y0 = -64;
		int z0 = portalOrigin.getZ()*8-128;
	...
	private void generateRangeMeshNether() {
		HashSet<Vec3d[]> quads = new HashSet<>();
		HashSet<Vec3d[]> lines = new HashSet<>();

		double x0 = portalOrigin.getX()/8.0d - 16;
		double y0 = 0;
		double z0 = portalOrigin.getZ()/8.0d - 16;
		for(int x = 0; x < voxels.length; x++) {
			for(int y = 0; y < voxels[0].length; y++) {
				for(int z = 0; z < voxels[0][0].length; z++) {
					if(!voxels[x][y][z]) continue;

					Vec3d v000 = new Vec3d(x0+x/8.0d,      y0+y,   z0+z/8.0d);
					Vec3d v100 = new Vec3d(x0+x/8.0d+.125, y0+y,   z0+z/8.0d);
					Vec3d v001 = new Vec3d(x0+x/8.0d,      y0+y,   z0+z/8.0d+.125);
					Vec3d v101 = new Vec3d(x0+x/8.0d+.125, y0+y,   z0+z/8.0d+.125);
					Vec3d v010 = new Vec3d(x0+x/8.0d,      y0+y+1, z0+z/8.0d);
					Vec3d v110 = new Vec3d(x0+x/8.0d+.125, y0+y+1, z0+z/8.0d);
					Vec3d v011 = new Vec3d(x0+x/8.0d,      y0+y+1, z0+z/8.0d+.125);
					Vec3d v111 = new Vec3d(x0+x/8.0d+.125, y0+y+1, z0+z/8.0d+.125);
